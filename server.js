...
if (requestComment && requestComment.username && database.articles[requestComment.articleId] &&
   database.users[requestComment.username] && requestComment.body) {
const comment = {
...
function deleteComment(url, request) {
    ...
    if (savedComment) {
      ...
      //database.users[comment.id] = null;
      //database.articles[comment.id] = null;

      const article = database.articles[savedComment.articleId];
      const articleComment = article && article.commentsId;
      ...
      const userComment = user && user.commentsId;
      ...
      response.status = 404;
      response.body = 'comment does not exist.';
    }
